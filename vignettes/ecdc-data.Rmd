...
covnat %>%
  mutate(cname = recode(cname, `United States` = "USA",
         ...
         cgroup = case_when(iso3 %in% focus_cn ~ iso3, 
                            TRUE ~ "ZZOTHER")) %>%
  mutate(cases7 = slider::slide_dbl((deaths/pop_2018)*1e6, mean, .before = 7, .after = 0, na.rm = TRUE),
         deaths7 = slider::slide_dbl((deaths/pop_2018)*1e6, mean, .before = 7, .after = 0, na.rm = TRUE)) %>%
