...
## NCHS Excess Death Estimates

The CDC's estimates of state-level excess deaths for the United States.

```{r nchswdc}
nchs_wdc
```

```{r cdcexcessplot, fig.height=10, fig.width=5, dpi=100}
nchs_wdc %>% 
  filter(jurisdiction %in% c("New Jersey", "New York",
                             "Michigan", "Georgia"), type == "Predicted (weighted)") %>% 
  group_by(jurisdiction, year, week) %>% 
  summarize(deaths = sum(average_number_of_deaths_in_time_period, na.rm = TRUE)) %>%
  group_by(year, week) %>%
  mutate(yr_ind = year %in% 2020) %>%
  ...
  facet_wrap(~ jurisdiction, ncol = 2, scales = "free_y") + 
  labs(x = "Week of the Year", 
       y = "Total Deaths", 
       color = "Years",
       title = "Weekly recorded deaths from all causes",
       subtitle = "Raw counts, each state has its own y-axis scale") + 
  theme_minimal() + 
  theme(legend.position = "top")
