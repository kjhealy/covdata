...
```{r cdcexcessplot, fig.height=7, fig.width=8, dpi=100}
nchs_wdc %>% 
  filter(jurisdiction %in% c("New York", "New Jersey", "Michigan", "Georgia", "California", "Alabama"), type != "Unweighted") %>%
  group_by(jurisdiction, year, week) %>% 
  summarize(deaths = sum(number_of_deaths, na.rm = TRUE)) %>%
  mutate(yr_ind = year %in% 2020) %>%
  ...
  labs(x = "Week of the Year", 
       ...
       subtitle = "2020 data are for Weeks 1 to 30. Raw Counts, each state has its own y-axis scale.", 
